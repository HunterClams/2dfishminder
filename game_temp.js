...
if (typeof vercelAnalytics !== 'undefined') {
    vercelAnalytics.track('Game Started');
}
...
function trackEntity(type, action, data = {}) {
    if (typeof vercelAnalytics !== 'undefined') {
        entityCounts[type] = entityCounts[type] || 0;
        if (action === 'spawned') {
            entityCounts[type]++;
        }
        vercelAnalytics.track('Entity Event', {
            entity_type: type,
            action: action,
            total_count: entityCounts[type],
            ...data
        });
    }
}
