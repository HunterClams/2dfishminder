...
class TrueFry1 extends Boid {
    ...
    draw() {
        ...
        // Apply reduced depth shader effects for TrueFry1 (50% less depth effect)
        const adjustedDepthY = this.y * 0.5; // Make appear 50% shallower for depth calculations
        const baseOpacity = window.Utils.getDepthOpacity(adjustedDepthY, 0.9) * 0.75; // 75% opacity
    ...
    drawFallback() {
        ...
        // Apply reduced depth shader effects to fallback drawing (50% less depth effect)
        const adjustedDepthY = this.y * 0.5; // Make appear 50% shallower for depth calculations
        const baseOpacity = window.Utils.getDepthOpacity(adjustedDepthY, 0.8) * 0.75; // 75% opacity
        const tintStrength = window.Utils.getDepthTint(adjustedDepthY);
